...
Screenshot and video capture for Godot projects. Platform-aware: detects macOS
vs Linux and uses the appropriate capture method.
...
On Linux, detect GPU display for hardware Vulkan:
```bash
GPU_DISPLAY=""
if [ "$OS_TYPE" = "Linux" ]; then
  for lock in /tmp/.X*-lock; do
    d=":${lock##/tmp/.X}"; d="${d%-lock}"
    if DISPLAY=$d timeout 2 glxinfo 2>/dev/null | ack -qi nvidia; then
      GPU_DISPLAY=$d; break
    fi
  done
fi
...
When `GPU_DISPLAY` is set, Godot uses hardware Vulkan with `--rendering-method forward_plus`.
...
if [ "$OS_TYPE" = "Darwin" ]; then
  # macOS: Godot runs natively with Metal/Vulkan, no xvfb needed
