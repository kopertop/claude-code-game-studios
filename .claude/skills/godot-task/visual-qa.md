...
If `GOOGLE_API_KEY` is not set, skip automated Visual QA entirely. Instead:
1. Capture screenshots normally (see capture.md)
2. Read the captured PNG files directly (Claude can view images)
3. Manually assess against the task's **Verify** criteria
4. Report pass/fail based on visual inspection of the screenshots
5. If uncertain, save screenshots and ask the user to review

This provides basic visual verification without the Gemini API.

## Static Mode (requires GOOGLE_API_KEY)
...
## Dynamic Mode (requires GOOGLE_API_KEY)
