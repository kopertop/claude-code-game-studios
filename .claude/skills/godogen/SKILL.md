...
Before running the pipeline, check which optional services are available:
- `GOOGLE_API_KEY` set → Gemini image generation and visual QA available
- `TRIPO3D_API_KEY` set → 3D model generation available
- Neither set → **No-API mode**: skip visual target, asset generation, and visual QA.
  Use placeholder assets (colored shapes, CSG primitives, Godot built-in meshes).
  The pipeline still works — it just produces programmer art instead of AI art.
...
User request
    ...
    +- Else:
    |   +- Skip visual target. Write ASSETS.md with "Art direction: placeholder/programmer art"
    |
    +- Decompose into tasks -> PLAN.md
    +- Design architecture -> STRUCTURE.md + project.godot + stubs
    |
    +- If budget provided AND GOOGLE_API_KEY is set (and no asset tables in ASSETS.md):
    |   +- Plan and generate assets -> ASSETS.md + updated PLAN.md with asset assignments
    +- Else:
    |   +- Use CSG primitives, colored MeshInstance3D, and built-in Godot resources as placeholders
    |
...
**No-API mode (GOOGLE_API_KEY not set):** Visual QA is skipped. Instead, rely on:
- Godot headless compilation check (`godot --headless --quit`) for script errors
- Test harness stdout for `ASSERT FAIL`
- Manual screenshot review by the user (capture PNGs and ask user to check)
...
The final task in PLAN.md is a presentation video — a script that showcases gameplay in a ~30-second cinematic MP4 (GPU required; skip on macOS or headless-only setups).
